...
(define (get-ch-words words ch)
  (filter (lambda (x) (and (not (null? x)) (eq? (first x) ch))) words))
...
(define (ast-func suffixes)
  (if (null? suffixes)
      '()
      (cons (list (first (car suffixes))) (map (lambda (x) (if (null? x) '() (cdr x))) suffixes))))
...
(define (cst-func suffixes)
  (cons  (longest-common-prefix-of-list suffixes) (map (lambda (x) (list-tail x (length (longest-common-prefix-of-list suffixes)))
                                                         )suffixes ))  )
  ;(cons  (longest-common-prefix-of-list suffixes) (length (longest-common-prefix-of-list suffixes) )  ))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (if (null? suffixes)
      '()
      (let* ((grouped (map (lambda (ch) (get-ch-words suffixes ch)) alphabet)) ;grupam dupa caracter de start
             (labeled (map labeling-func grouped))) ; apelez functiile create mai sus pt fiecare tip de parametru
        (foldr (lambda (branch st) ;iau ramurile, in ordinea buna fac subtreeul recursiv si adaug la suff tree
                 (if (or (null? branch) (null? (car branch))) ; daca ramura e goala am terminat
                     st
                     (cons (cons (car branch) (suffixes->st labeling-func (cdr branch) alphabet)) st))) ; adaug subarbore la arbore de suf
               '()
               labeled))
      )
  )
...
(define (text->st labeling-func) ; curried : doar un argument pe rand
  (lambda (text)
    (let* ((suffixes (get-suffixes (append text (list #\$)))) ; toate sufixele si adaug la final $
           (alphabet (sort (remove-duplicates (append text (list #\$))) char<?))) ; alfabetul (literle) fara duplicate din text
...
; asociat textului.
(define text->ast (text->st ast-func)) ; se reapeleaza fct de mai sus, doar ca cu labelingul dorit
