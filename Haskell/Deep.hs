...
toTransformation (Combine []) = id -- transformarea identitate
toTransformation (Combine (t:ts)) = toTransformation (Combine ts) . toTransformation t
...
decomposeTransformation = go
  where
    go (Translation tx ty) = [Translation tx ty]
    go (Scaling factor) = [Scaling factor]
    go (Combine transformations) = concatMap go transformations -- concatenam listele de transformari
...
fuseTransformations = foldr step []
  where
    step t1 (t2:ts) = case (t1, t2) of
      (Translation tx1 ty1, Translation tx2 ty2) -> Translation (tx1 + tx2) (ty1 + ty2) : ts
      (Scaling s1, Scaling s2) -> Scaling (s1 * s2) : ts
      _ -> t1 : t2 : ts
    step t [] = [t]
