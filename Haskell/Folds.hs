...
instance Show RegionAST where
    show = showIndented 0
      where
        showIndented level (R (FromPoints points)) = replicate (2 * level) ' ' ++ "FromPoints " ++ show points
        showIndented level (R (Rectangle w h)) = replicate (2 * level) ' ' ++ "Rectangle " ++ show w ++ " " ++ show h
        showIndented level (R (Circle r)) = replicate (2 * level) ' ' ++ "Circle " ++ show r
        showIndented level (R (Transform t r)) = replicate (2 * level) ' ' ++ show t ++ "\n" ++ showIndented (level + 1) r
        showIndented level (R (Complement r)) = replicate (2 * level) ' ' ++ "~" ++ "\n" ++ showIndented (level + 1) r
        showIndented level (R (Union r1 r2)) = replicate (2 * level) ' ' ++ "+" ++ "\n" ++ showIndented (level + 1) r1 ++ "\n" ++ showIndented (level + 1) r2
        showIndented level (R (Intersection r1 r2)) = replicate (2 * level) ' ' ++ "*" ++ "\n" ++ showIndented (level + 1) r1 ++ "\n" ++ showIndented (level + 1) r2
{-
    *** TODO ***
...
instance Functor RegionShape where
  ...
  fmap _ (Rectangle w h) = Rectangle w h
  fmap _ (FromPoints ps) = FromPoints ps
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Union reg1 reg2) = (fst reg1 + fst reg2, snd reg1 + snd reg2)
